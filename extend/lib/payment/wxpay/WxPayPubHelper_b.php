...
class Common_util_pub {
    ...
    public function getSign($Obj) {
        ...
        //签名步骤二：在string后加入KEY
        $String = $String . "&key=" . WxPayConf_pub::KEY_B;
    ...
    function postXmlSSLCurl($xml, $url, $second = 30) {
        ...
        curl_setopt($ch, CURLOPT_SSLCERT, WxPayConf_pub::SSLCERT_PATH_B);
        //默认格式为PEM，可以注释
        curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
        curl_setopt($ch, CURLOPT_SSLKEY, WxPayConf_pub::SSLKEY_PATH_B);
        //post提交方式
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
        $data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub {
     ...
     */
    function createXml() {
        $this->parameters["appid"] = WxPayConf_pub::APPID_B; //公众账号ID
        $this->parameters["mch_id"] = WxPayConf_pub::MCHID_B; //商户号
...
class NativeCall_pub extends Wxpay_server_pub {
    ...
    function createXml() {
        if ($this->returnParameters["return_code"] == "SUCCESS") {
            $this->returnParameters["appid"] = WxPayConf_pub::APPID_B; //公众账号ID
            $this->returnParameters["mch_id"] = WxPayConf_pub::MCHID_B; //商户号
...
class JsApi_pub extends Common_util_pub {
    ...
    function createOauthUrlForCode($redirectUrl, $state) {
        $urlObj["appid"] = WxPayConf_pub::APPID_B;
    ...
    function createOauthUrlForOpenid() {
        $urlObj["appid"] = WxPayConf_pub::APPID_B;
        $urlObj["secret"] = WxPayConf_pub::APPSECRET_B;
    ...
    public function getParameters() {
        $jsApiObj["appId"] = WxPayConf_pub::APPID_B;
        $timeStamp = time();
